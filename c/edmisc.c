 ...
 *
 *  \author Dave G. Conroy, Gary D. Riley, Steve Wilhite, George Jones, Chris Culbert
 *  \version 0.0.1
 *  \note
 *    [base edmisc.c clips_621]
 *
 **/
#include "setup.h"
...
#pragma argsused
#endif
globle int compile_region(void *env, int f, int n)
{
    register int s;
    REGION region;
    ...
    /* Create IO router for the region (Ix.C) */

    ix_rtr_add(env, "emacs_region", 90, region_fnd,
                 NULL, region_getc, region_ungetc, NULL);

    /* COMPILE */

    if( get_compile(env, "emacs_region", "Emacs_region") == 0 )
    {
...
#pragma argsused
#endif
globle int compile_file(void *env, int f, int n)
{
    if( curbp == CompileBufferp )
    {
    ...
    /*  Create a IO router for the file   (Ix.C)  */

    ix_rtr_add(env, "emacs_file", 90, buffer_fnd,
                 NULL, buffer_getc, buffer_ungetc, NULL);

    /*   COMPILE   */

    if( get_compile(env, "emacs_file", "Emacs_buffer") == 0 )
    {
        mlwrite("Error while forming compilation buffer!");
    }
    else
 ...
 **********************************************************/
globle int get_compile(void *env, char *str1, char *str2)
{
    ...
    if( bclear(env, CompileBufferp) != TRUE )      /* Blow old text away   */
    {
        return(0);
    }
    ...
    ix_rtr_activate(env, str1);
    ix_rtr_activate(env, "cmp_router");
    ix_constructs_is_verbose_load_set(env, TRUE);
    ix_construct_load_from_logical_name(env, str2);
    ix_pp_destroy(env);

    /* Flush last diagnostic line (if any) to buffer */
    if( CompileLineIndex != 0 )
    {
        addline(env, CompileBufferp, CompileLine);
    }

    ix_rtr_deactivate(env, str1);
    ix_rtr_deactivate(env, "cmp_router");
    ix_constructs_is_verbose_load_set(env, FALSE);
    ix_rtr_delete(env, str1);

    sys_strcpy(CompileBufferp->b_fname, "");

    if( CompileBufferp->b_nwnd == 0 )          /* Not on screen yet.   */
    {
        if((wp = wpopup(env)) == NULL )
        {
            return(0);
        }
...
globle int region_fnd(void *env, char *log_name)
{
...
#pragma argsused
#endif
globle int region_getc(void *env, char *log_name)
{
    int c;
...
#pragma argsused
#endif
globle int region_ungetc(void *env, int c, char *log_name)
...
globle int buffer_fnd(void *env, char *log_name)
{
...
#pragma argsused
#endif
globle int buffer_getc(void *env, char *log_name)
{
    int c;
...
#pragma argsused
#endif
globle int buffer_ungetc(void *env, int c, char  *logical_name)
{
    if( c == EOF )
    {
        return(1);
...
globle int query_cmp(void *env, char *logName)
{
...
globle int print_cmp(void *env, char *logName, char *str)
{
    register int i;
...
globle void init_cmp_router(void *env)
{
    ix_rtr_add(env, "cmp_router",
                 20,
                 query_cmp,
                 print_cmp,
                 NULL,
...
globle void kill_cmp_router(void *env)
{
    ix_rtr_delete(env, "cmp_router");
}
...
#pragma argsused
#endif
globle int setfillcol(void *env, int f, int n)
{
    fillcol = n;
    return(TRUE);
}
...
#pragma argsused
#endif
globle int showcpos(void *env, int f, int n)
{
    register int cline;
    register int col;
    register int tline;
...
#pragma argsused
#endif
globle int gotoline(void *env, int f, int n)
{
    register int line;
    char buf[5];
    register int i;
    ...
    if((s = mlreply(env, "Goto line: ", buf, 5)) != TRUE )
    {
        return(s);
    }
...
#pragma argsused
#endif
globle int twiddle(void *env, int f, int n)
{
    register LINE   *dotp;
    register int doto;
    register int cl;
...
#pragma argsused
#endif
globle int quote(void *env, int f, int n)
{
    register int s;
    register int c;
...
#pragma argsused
#endif
globle int tab(void *env, int f, int n)
{
    if( n < 0 )
    {
        return(FALSE);
    ...
    if( !tabsize )
    {
        return(linsert(env, 1, '\t'));
    }

    return(linsert(env, tabsize - (getccol(FALSE) % tabsize), ' '));
}
...
#pragma argsused
#endif
globle int openline(void *env, int f, int n)
{
    register int i;
    register int s;
    ...
    {
        s = backchar(env, f, n);                    /* of them all.         */
    }
...
#pragma argsused
#endif
globle int newline(void *env, int f, int n)
{
    register LINE   *lp;
    register int s;
    ...
    while( n-- )
    {
        ...
        if((llength(lp) == curwp->w_doto)
           ...
           && (llength(lforw(lp)) == 0))
        {
            if((s = forwchar(env, FALSE, 1)) != TRUE )
            {
                return(s);
            }
        }
        else if((s = lnewline(env)) != TRUE )
        {
            return(s);
        }
    }
...
#pragma argsused
#endif
globle int deblank(void *env, int f, int n)
{
    register LINE   *lp1;
    register LINE   *lp2;
    long nld;
...
#pragma argsused
#endif
globle int indent(void *env, int f, int n)
{
    register int nicol;
    register int c;
    register int i;
    ...
    while( n-- )
    {
        ...
        if((lnewline(env) == FALSE)
           || (((i = nicol / 8) != 0) && (linsert(env, i, '\t') == FALSE))
           || (((i = nicol % 8) != 0) && (linsert(env, i,  ' ') == FALSE)))
        {
            return(FALSE);
        }
...
#pragma argsused
#endif
globle int forwdel(void *env, int f, int n)
{
    if( n < 0 )
    {
        return(backdel(env, f, -n));
    }
...
#pragma argsused
#endif
globle int backdel(void *env, int f, int n)
{
    register int s;

    if( n < 0 )
    {
        return(forwdel(env, f, -n));
    }
    ...
    if((s = backchar(env, f, n)) == TRUE )
    {
        s = ldelete(env, (long)n, f);
    }
...
#pragma argsused
#endif
globle int kill_fwd(void *env, int f, int n)
{
    register int chunk;
    register LINE   *nextp;
    ...
    {
        kdelete(env);                              /* last wasn't a kill.  */
    }
    ...
    return(ldelete(env, (long)chunk, TRUE));
}
...
#pragma argsused
#endif
globle int yank(void *env, int f, int n)
{
    register int c;
    register int i;
...
#pragma argsused
#endif
globle int forwsearch(void *env, int f, int n)
{
    register LINE *clp;
    register int cbo;
    register LINE*tlp;
    ...
    if((s = readpattern(env, "Search")) != TRUE )
    {
        return(s);
    }
...
#pragma argsused
#endif
globle int backsearch(void *env, int f, int n)
{
    register LINE *clp;
    register int cbo;
    register LINE *tlp;
    ...
    if((s = readpattern(env, "Reverse search")) != TRUE )
    {
        return(s);
    }
...
#pragma argsused
#endif
globle int bkwrdrpl(void *env, int f, int n)
{
    LINE *clp;
    int cbo;
    LINE *tlp;
    ...
    if((s = readpattern(env, "Rev. replace all occrs of")) != TRUE )
    {
        return(s);
    }

    pat2 = (char *)ix_malloc(env, (unsigned)strlen(pat) + 1);          /* Pat2 is first pattern */
    sys_strcpy(pat2, pat);

    if((s = mlreply(env, "Replace with: ", pat, NPAT)) == ABORT )
    {
        ix_free(env, (void *)pat2, (unsigned)strlen(pat) + 1);
        return(s);
    }
    ...
    curwp->w_flag |= WFMOVE;
    update();
    sys_sprintf(buf, " %d Replacement[s] ", count);
    mlwrite(buf);
    return(TRUE);
}
...
#pragma argsused
#endif
globle int bkwrdcr(void *env, int f, int n)
{
    LINE *clp;
    int cbo;
    LINE *tlp;
    ...
    if((s = readpattern(env, "Rev. replace some occrs. of")) != TRUE )
    {
        return(s);
    }

    pat2 = (char *)ix_malloc(env, (unsigned)strlen(pat) + 1);
    sys_strcpy(pat2, pat);
...
#pragma argsused
#endif
globle int frwsr(void *env, int f, int n)
{
    LINE *clp, *tlp;
    ...
    if((s = readpattern(env, "Replace the occurences of?")) != TRUE )
    {
        return(s);
    }
...
#pragma argsused
#endif
globle int querysr(void *env, int f, int n)
{
    LINE *clp, *tlp;
    int cbo, tbo, c, s;
    char *pp;
    ...
    if((s = readpattern(env, "Query_replace ?")) != TRUE )
    {
        return(s);
    }
...
globle int lreplace(void *env, char *pat2)
{
    int doto;
    unsigned i;
    char *cp1, *cp2;
    ...
    if((lp2 = lalloc(env, (int)(lp1->l_used - strlen(pat2) + strlen(pat) ))) == NULL )
    {
        return(FALSE);
    }
    ...
    *cp2 = *cp1;
    ...
    ix_free(env, (void *)lp1, (unsigned)sizeof(LINE) + lp1->l_size);
    return(TRUE);
}
...
#pragma argsused
#endif
globle int smatchb(void *env, int f, int n)
{
    int cbo, c;
    LINE *clp;
...
globle int searchcl(int tempch)
{
    LINE *lp;
    int tbo, c, i = 1;
...
globle int searchop(int tempch)
{
    LINE *lp;
    int tbo, c, i = 1;
 ...
 */
globle int readpattern(void *env, char *prompt)
{
    register char *cp1;
    register char *cp2;
    register int c;
    ...
    *cp1++ = ' ';
    *cp1++ = '\0';
    s = mlreply(env, tpat, tpat, NPAT);      /* Read pattern */
...
#pragma argsused
#endif
globle int spawncli(void *env, int f, int n)
{
...
#pragma argsused
#endif
globle int spawn(void *env, int f, int n)
{
    register int s;
    char line[NLINE];
    ...
    if((s = mlreply(env, "DCL command: ", line, NLINE)) != TRUE )
    {
        return(s);
    }
    ...
    if((s = mlreply(env, "MS-DOS command: ", line, NLINE)) != TRUE )
    {
        return(s);
    }
    ...
    if((s = mlreply(env, "! ", line, NLINE)) != TRUE )
    {
        return(s);
    }
