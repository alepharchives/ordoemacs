 ...
 *  \brief MicroEmacs
 *
 *  \author Dave G. Conroy, Gary D. Riley
 *  \version 0.0.1
 *  \note
 *    [base edbasic.c clips_621]
 *
 **/
#include "setup.h"
...
#pragma argsused
#endif
globle int gotobol(void *env, int f, int n)
{
    curwp->w_doto  = 0;
    return(TRUE);
}
 ...
 */
globle int backchar(void *env, int f, int n)
{
    register LINE   *lp;

    if( n < 0 )
    {
        return(forwchar(env, f, -n));
    }
...
#pragma argsused
#endif
globle int gotoeol(void *env, int f, int n)
{
 ...
 */
globle int forwchar(void *env, int f, int n)
{
    if( n < 0 )
    {
        return(backchar(env, f, -n));
    }
...
#pragma argsused
#endif
globle int gotobob(void *env, int f, int n)
{
...
#pragma argsused
#endif
globle int gotoeob(void *env, int f, int n)
{
 ...
 * possible.
 */
globle int forwline(void *env, int f, int n)
{
    register LINE   *dlp;

    if( n < 0 )
    {
        return(backline(env, f, -n));
    }
 ...
 */
globle int backline(void *env, int f, int n)
{
    register LINE   *dlp;

    if( n < 0 )
    {
        return(forwline(env, f, -n));
    }
 ...
 */
globle int forwpage(void *env, int f, int n)
{
    register LINE   *lp;
 ...
 * reason.
 */
globle int backpage(void *env, int f, int n)
{
    register LINE   *lp;
...
#pragma argsused
#endif
globle int setmark(void *env, int f, int n)
{
...
#pragma argsused
#endif
globle int swapmark(void *env, int f, int n)
{
    register LINE   *odotp;
    register int odoto;
...
globle int wrapword(void *env)
{
    ...
    return(forwword(env, 0, 1) && forwchar(env, 0, cnt));
}
...
#pragma argsused
#endif
globle int backword(void *env, int f, int n)
{
    if( n < 0 )
    {
        return(forwword(env, f, -n));
    }

    if( backchar(env, FALSE, 1) == FALSE )
    {
        return(FALSE);
    }
...
#pragma argsused
#endif
globle int forwword(void *env, int f, int n)
{
    if( n < 0 )
    {
        return(backword(env, f, -n));
    }
...
#pragma argsused
#endif
globle int upperword(void *env, int f, int n)
{
    register int c;
...
#pragma argsused
#endif
globle int lowerword(void *env, int f, int n)
{
    register int c;
...
#pragma argsused
#endif
globle int capword(void *env, int f, int n)
{
    register int c;
...
#pragma argsused
#endif
globle int delfword(void *env, int f, int n)
{
    register int size;
    register LINE   *dotp;
    register int doto;
    ...
    curwp->w_dotp = dotp;
    curwp->w_doto = doto;
    return(ldelete(env, (long)size, TRUE));
}
...
#pragma argsused
#endif
globle int delbword(void *env, int f, int n)
{
    register int size;
...
#pragma argsused
#endif
globle int killregion(void *env, int f, int n)
{
    register int s;
    REGION region;
    ...
    return(ldelete(env, region.r_size, TRUE));
}
...
#pragma argsused
#endif
globle int copyregion(void *env, int f, int n)
{
    register LINE   *linep;
    register int loffs;
    register int s;
    ...
    while( region.r_size-- )
    {
        ...
        {
            if((s = kinsert(env, lgetc(linep, loffs))) != TRUE )
            {
                return(s);
            }
...
#pragma argsused
#endif
globle int lowerregion(void *env, int f, int n)
{
    register LINE   *linep;
    register int loffs;
    register int c;
...
#pragma argsused
#endif
globle int upperregion(void *env, int f, int n)
{
    register LINE   *linep;
    register int loffs;
    register int c;
 ...
 * conform thing later.
 */
globle int getregion(REGION *rp)
{
    register LINE   *flp;
    register LINE   *blp;
    long fsize;
...
#pragma argsused
#endif
globle int fileread(void *env, int f, int n)
{
    register int s;
    char fname[NFILEN];

    if((s = mlreply(env, "Visit file: ", fname, NFILEN)) != TRUE )
    {
        return(s);
    }

    return(readin(env, fname));
}
...
#pragma argsused
#endif
globle int filevisit(void *env, int f, int n)
{
    register int s;
    char fname[NFILEN];

    if((s = mlreply(env, "Find file: ", fname, NFILEN)) != TRUE )
    {
        return(s);
    }

    filevisit_guts(env, fname);
    return(TRUE);
}

globle int filevisit_guts(void *env, char fname[])
{
    register BUFFER *bp;
    register WINDOW *wp;
    register LINE   *lp;
    ...
    while((bp = bfind(env, bname, FALSE, 0)) != NULL )
    {
        s = mlreply(env, "Buffer name: ", bname, NBUFN);
    ...
    if((bp == NULL) && ((bp = bfind(env, bname, TRUE, 0)) == NULL))
    {
    ...
    sys_strcpy(lastbufn, curbp->b_bname);           /* Set last buufer name */

    curbp = bp;                                 /* Switch to it.        */
    curwp->w_bufp = bp;
    curbp->b_nwnd++;
    return(readin(env, fname));                     /* Read it in.          */
}
 ...
 * an argument.
 */
globle int readin(void *env, char fname[])
{
    register LINE   *lp1;
    register LINE   *lp2;
    register int i;
    ...
    if((s = bclear(env, bp)) != TRUE )              /* Might be old.        */
    {
        return(s);
    }

    bp->b_flag &= ~(BFTEMP | BFCHG);
    sys_strcpy(bp->b_fname, fname);
 ...
 */
globle int makename(char bname[], char fname[])
{
    register char   *cp1;
    register char   *cp2;
...
#pragma argsused
#endif
globle int filewrite(void *env, int f, int n)
{
    register WINDOW *wp;
    register int s;
    char fname[NFILEN];
    ...
    sys_sprintf(prompt, "Write file [%s]: ", curbp->b_fname);

    if((s = mlreply(env, prompt, fname, NFILEN)) != TRUE )
    {
        if( s == FALSE )
        {
            sys_strcpy(fname, curbp->b_fname);
        }
        else
        {
            return(s);
    ...
    {
        sys_strcpy(curbp->b_fname, fname);
        curbp->b_flag &= ~BFCHG;
...
#pragma argsused
#endif
globle int filesave(void *env, int f, int n)
{
    register WINDOW *wp;
    register int s;
...
#pragma argsused
#endif
globle int filename(void *env, int f, int n)
{
    register WINDOW *wp;
    register int s;
    char fname[NFILEN];

    if((s = mlreply(env, "Name: ", fname, NFILEN)) == ABORT )
    {
        return(s);
    }
 ...
 */
globle int ffputline(char buf[], int nbuf)
{
    register int i;
 ...
 */
globle int ffgetline(char buf[], int nbuf)
{
    register int c;
    register int i;
