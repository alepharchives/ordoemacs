...
#pragma argsused
#endif
globle int usebuffer(void *env, int f, int n)
{
    register BUFFER *bp;
    register WINDOW *wp;
    register int s;
    ...
    sys_sprintf(prompt, "Use buffer [%s]: ", lastbufn);

    if((s = mlreply(env, prompt, bufn, NBUFN)) != TRUE )
    {
        if((s == FALSE) && (strlen(lastbufn) != 0))
        {
            sys_strcpy(bufn, lastbufn);
        }
        else
        {
            return(s);
    ...
    if((bp = bfind(env, bufn, TRUE, 0)) == NULL )
    {
        return(FALSE);
    }

    sys_strcpy(lastbufn, curbp->b_bname);           /* Save current bufname */
...
#pragma argsused
#endif
globle int killbuffer(void *env, int f, int n)
{
    register BUFFER *bp;
    register BUFFER *bp1;
    register BUFFER *bp2;
    ...
    if((s = mlreply(env, "Kill buffer: ", bufn, NBUFN)) != TRUE )
    {
        return(s);
    }

    if((bp = bfind(env, bufn, FALSE, 0)) == NULL )  /* Easy if unknown.     */
    {
        return(TRUE);
    }
    ...
    if((s = bclear(env, bp)) != TRUE )              /* Blow text away.      */
    {
        return(s);
    }

    ix_free(env, (void *)bp->b_linep, (unsigned)(sizeof(LINE) + bp->b_linep->l_size));
    ...
    ix_free(env, (void *)bp, (unsigned)sizeof(BUFFER));        /* Release buffer block */
...
#pragma argsused
#endif
globle int listbuffers(void *env, int f, int n)
{
    register WINDOW *wp;
    register BUFFER *bp;
    register int s;

    if((s = makelist(env)) != TRUE )
    {
        return(s);
    }
...
globle int makelist(void *env)
{
    ...
    if((s = bclear(env, blistp)) != TRUE )          /* Blow old text away   */
    {
        return(s);
    }

    sys_strcpy(blistp->b_fname, "");

    if((addline(env, blistp, "C   Size Buffer           File") == FALSE)
       || (addline(env, blistp, "-   ---- ------           ----") == FALSE))
    {
        return(FALSE);
    }
    ...
    while( bp != NULL )
    {
        ...
        *cp1 = 0;                               /* Add to the buffer.   */
        ...
        if( addline(env, blistp, line) == FALSE )
        {
            return(FALSE);
        }
 ...
 */
globle int addline(void *env, BUFFER *bufferp, char    *text)
{
    register LINE   *lp;
    register int i;
    register int ntext;
 ...
 */
globle BUFFER  *bfind(void *env, char   *bname, int cflag, int bflag)
{
    register BUFFER *bp;
    register LINE   *lp;
    ...
    if( cflag != FALSE )
    {
        if((bp = (BUFFER *)ix_malloc(env, (unsigned)sizeof(BUFFER))) == NULL )
        {
            return(NULL);
        }

        if((lp = lalloc(env, 0)) == NULL )
        {
            ix_free(env, (void *)bp, (unsigned)sizeof(BUFFER));
            return(NULL);
        }
        ...
        bp->b_nwnd  = 0;
        bp->b_linep = lp;
        sys_strcpy(bp->b_fname, "");
        sys_strcpy(bp->b_bname, bname);
        lp->l_fp = lp;
        lp->l_bp = lp;
    }
 ...
 * looks good.
 */
globle int bclear(void *env, BUFFER *bp)
{
    register LINE   *lp;
    register int s;
    ...
    if(((bp->b_flag & BFTEMP) == 0)             /* Not scratch buffer.  */
       ...
       && ((s = mlyesno(env, "Discard changes")) != TRUE))
    {
        return(s);
    }
 ...
 */
globle LINE    *lalloc(void *env, int used)
{
    register LINE   *lp;
    register int size;
    ...
    lp = (LINE *)ix_malloc(env, (unsigned)sizeof(LINE) + size);
    lp->l_size = size;
    lp->l_used = used;
    return(lp);
}
 ...
 */
globle void lfree(void *env, LINE   *lp)
{
    register BUFFER *bp;
    register WINDOW *wp;
    ...
    ix_free(env, (void *)lp, (unsigned)(sizeof(LINE) + lp->l_size));
}
 ...
 */
globle int linsert(void *env, int n, int c)
{
    register char   *cp1;
    register char   *cp2;
    register LINE   *lp1;
    ...
    if( lp1 == curbp->b_linep )                 /* At the end: special  */
    {
        ...
        if((lp2 = lalloc(env, n)) == NULL )         /* Allocate new line    */
        {
            return(FALSE);
        }
    ...
    {
        if((lp2 = lalloc(env, lp1->l_used + n)) == NULL )
        {
            return(FALSE);
        }
...
globle int lnewline(void *env)
{
    ...
    if((lp2 = lalloc(env, doto)) == NULL )          /* New first half line  */
    {
        return(FALSE);
    }
 ...
 */
globle int ldelete(void *env, long n, int kflag)
{
    register char   *cp1;
    register char   *cp2;
    register LINE   *dotp;
    ...
    while( n != 0 )
    {
        ...
        if( chunk == 0 )                        /* End of line, merge.  */
        {
            ...
            if((ldelnewline(env) == FALSE)
               || ((kflag != FALSE) && (kinsert(env, '\n') == FALSE)))
            {
                return(FALSE);
            }
 ...
 * "ldelete" only.
 */
globle int ldelnewline(void *env)
{
    register char   *cp1;
    register char   *cp2;
    register LINE   *lp1;
    ...
    if( lp2->l_used <= lp1->l_size - lp1->l_used )
    {
        ...
        lp1->l_fp = lp2->l_fp;
        lp2->l_fp->l_bp = lp1;
        ix_free(env, (void *)lp2, (unsigned)(sizeof(LINE) + lp2->l_size));
        return(TRUE);
    }

    if((lp3 = lalloc(env, lp1->l_used + lp2->l_used)) == NULL )
    {
        return(FALSE);
    }
 ...
 */
globle void kdelete(void *env)
{
    if( kbufp != NULL )
    {
        ix_free(env, (void *)kbufp, (unsigned)ksize);
        kbufp = NULL;
        kused = 0;
        ksize = 0;
    }
 ...
 */
globle int kinsert(void *env, int c)
{
#if  WIN_MVC || WIN_BTC
    char *nbufp;
#else
    ...
    if( kused == ksize )
    {
        if((nbufp = (char *)ix_malloc(env, (unsigned)ksize + KBLOCK)) == NULL )
        {
            return(FALSE);
        }
...
#pragma argsused
#endif
globle int reposition(void *env, int f, int n)
{
...
#pragma argsused
#endif
globle int EditorRefresh(void *env, int f, int n)
{
    if( f == FALSE )
    {
        sgarbf = TRUE;
...
#pragma argsused
#endif
globle int nextwind(void *env, int f, int n)
{
    register WINDOW *wp;
...
#pragma argsused
#endif
globle int prevwind(void *env, int f, int n)
{
    register WINDOW *wp1;
    register WINDOW *wp2;
 ...
 */
globle int mvdnwind(void *env, int f, int n)
{
    return(mvupwind(env, f, -n));
}
...
#pragma argsused
#endif
globle int mvupwind(void *env, int f, int n)
{
    register LINE *lp;
    register int i;
...
#pragma argsused
#endif
globle int onlywind(void *env, int f, int n)
{
    register WINDOW *wp;
    register LINE   *lp;
    register int i;
    ...
    while( wheadp != curwp )
    {
        ...
        ix_free(env, (void *)wp, (unsigned)sizeof(WINDOW));
    }
...
#pragma argsused
#endif
globle int splitwind(void *env, int f, int n)
{
    register WINDOW *wp;
    register LINE   *lp;
    register int ntru;
    ...
    if((wp = (WINDOW *)ix_malloc(env, (unsigned)sizeof(WINDOW))) == NULL )
    {
 ...
 * move. Bound to "C-X Z".
 */
globle int enlargewind(void *env, int f, int n)
{
 ...
 * "C-X C-Z".
 */
globle int shrinkwind(void *env, int f, int n)
{
    register WINDOW *adjwp;
    register LINE   *lp;
    register int i;
 ...
 */
globle WINDOW  *wpopup(void *env)
{
    register WINDOW *wp;

    if((wheadp->w_wndp == NULL)                 /* Only 1 window        */
       && (splitwind(env, FALSE, 0) == FALSE))      /* and it won't split   */
    {
        return(NULL);
    }
...
globle void vtinit(void *env)
{
    ...
    (*term.t_open)();
    vscreen = (VIDEO **)ix_malloc(env, (unsigned)term.t_nrow * sizeof(VIDEO *));
    ...
    pscreen = (VIDEO **)ix_malloc(env, (unsigned)term.t_nrow * sizeof(VIDEO *));
    ...
    {
        vp = (VIDEO *)ix_malloc(env, (unsigned)sizeof(VIDEO) + term.t_ncol);
 ...
 */
globle void vtmove(int row, int col)
{
    vtrow = row;
    vtcol = col;
}
 ...
 */
globle void updateline(int row, char vline[], char pline[])
{
    register char *cp1;
    register char *cp2;
    register char *cp3;
 ...
 */
globle void modeline(WINDOW *wp)
{
    register char *cp;
    register int c;
    register int n;
 ...
 */
globle void movecursor(int row, int col)
{
 ...
 */
globle int mlyesno(void *env, char *prompt)
{
    register int s;
    char buf[64];
    ...
    for(;;)
    {
        ...
        sys_strcat(buf, " [y/n]? ");
        s = mlreply(env, buf, buf, sizeof(buf));
 ...
 */
globle int mlreply(void *env, char *prompt, char *buf, int nbuf)
{
    register int cpos;
    register int i;
    register int c;
 ...
 */
globle void mlputi(int i, int r)
{
    register int q;
 ...
 */
globle void mlputli(long l, int r)
{
    register long q;
...
globle void kill_video_buffers(void *env)
{
    int i;

    for( i = 0; i < term.t_nrow; ++i )
    {
        ix_free(env, (void *)vscreen[i], (unsigned)(sizeof(VIDEO) + term.t_ncol));
        ix_free(env, (void *)pscreen[i], (unsigned)(sizeof(VIDEO) + term.t_ncol));
    }

    ix_free(env, (void *)vscreen, (unsigned)(term.t_nrow * sizeof(VIDEO *)));
    ix_free(env, (void *)pscreen, (unsigned)(term.t_nrow * sizeof(VIDEO *)));
}

static void int_to_ascii(char buf[], int width, int num)
{
