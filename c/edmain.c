...
static void PerformEditCommand(void *env)
{
    register int c;
    register int f;
    register int n;
    ...
    int num_a;
    char *fileName = NULL;
    DATA_OBJECT_T arg_ptr;

    /*====================
     * Get the file name.
     *====================*/

    if((num_a = ix_arg_count_chk(env, "editor", NO_MORE_THAN, 1)) == -1 )
    {
        return;
    }
    ...
    if( num_a == 1 )
    {
        if( ix_arg_type_chk(env, "editor", 1, SYMBOL_OR_STRING, &arg_ptr) == FALSE )
        {
            return;
        }

        fileName = to_string(arg_ptr);
    }
    ...
    if( bheadp == NULL )
    {
        ...
        sys_strcpy(bname, "main");                  /* Work out the name of */
        ...
        edinit(env, bname);                          /* Buffers, windows.    */
        vtinit(env);                               /* Displays.            */

        if( num_a > 0 )
        {
            update();                           /* You have to update   */
            readin(env, fileName);                /* in case "[New file]" */
        }

        init_cmp_router(env);        /* Prepare the compile  */
        ix_rtr_deactivate(env, "cmp_router");        /* router.              */
    }
    else
    {
        /*********************************************************
...
loop:
    ...
    rtn_flag = execute(env, c, f, n);                   /* Do it.               */
 ...
 */
globle void edinit(void *env, char bname[])
{
    register BUFFER *bp;
    register WINDOW *wp;

    bp     = bfind(env, bname, TRUE, 0);                 /* First buffer        */
    blistp = bfind(env, "[List]", TRUE, BFTEMP);        /* Buffer list buffer  */
    wp     = (WINDOW *)ix_malloc(env, (unsigned)sizeof(WINDOW));       /* First window        */
    ...
    CompileBufferp = bfind(env, "[Compilations]", TRUE, BFTEMP);
}
 ...
 */
globle int execute(void *env, int c, int f, int n)
{
    register KEYTAB *ktp;
    register int status;
 ...
 */
globle int quickexit(void *env, int f, int n)
{
    ...
    return(edquit(env, f, n));                   /* conditionally quit   */
}
...
#pragma argsused
#endif
globle int edquit(void *env, int f, int n)
{
    register int s;
    ...
    if((f != FALSE)                             /* Argument forces it.  */
       || (anycb() == FALSE)                    /* All buffers clean.
                                                 * User says it's OK.   */
       || ((s = mlyesno(env, "Modified Buffers! Quit")) == TRUE))
    {
        vttidy();
        full_cleanup(env);
        return(EXIT);
...
#pragma argsused
#endif
globle int temp_quit(void *env, int f, int n)
{
    vttidy();
    return(EXIT);
}
...
#pragma argsused
#endif
globle int ctlxlp(void *env, int f, int n)
{
...
#pragma argsused
#endif
globle int ctlxrp(void *env, int f, int n)
{
    if( kbdmip == NULL )
    {
        mlwrite("Not now");
...
#pragma argsused
#endif
globle int ctlxe(void *env, int f, int n)
{
    register int c;
    register int af;
    register int an;
    ...
    do
    {
        ...
        do
        {
            ...
            s = TRUE;
        } while( c != (CTLX | ')') && (s = execute(env, c, af, an)) == TRUE );
...
#pragma argsused
#endif
globle int ctrlg(void *env, int f, int n)
{
    (*term.t_beep)();
...
globle void full_cleanup(void *env)
{
/*   Clear all data structures */

    kill_all_buffers(env, &bheadp);   /* Clear all existing buffers   */

    kill_all_windows(env);          /* Clear all windows            */

    kill_video_buffers(env);  /* Kill special video buffers   */

    kill_cmp_router(env);     /* Get rid of special router    */
...
globle int kill_all_buffers(void *env, BUFFER **top_buf)
{
    register BUFFER *bp;
    ...
    while( bp != NULL )
    {
        spec_clear(env, bp);                         /* Blow text away.      */

        ix_free(env, (void *)bp->b_linep,           /* And free pointer     */
...
globle int kill_all_windows(void *env)
{
    register WINDOW *wp;
    register WINDOW *wp1;
...
globle int spec_clear(void *env, BUFFER *bp)
{
    register LINE   *lp;
...
globle void EditCommand(void *env)
{
    void (*redrawScreenFunction)(void *);
    void (*pauseEnvFunction)(void *);
    void (*continueEnvFunction)(void *, int);

    redrawScreenFunction = sys_redraw_fn_get(env);
    pauseEnvFunction = sys_pause_fn_get(env);
    continueEnvFunction = sys_resume_fn_get(env);

    if( pauseEnvFunction != NULL )
    {
        (*pauseEnvFunction)(env) ;
    }

    PerformEditCommand(env);

    if( continueEnvFunction != NULL )
    {
        (*continueEnvFunction)(env, 0) ;
    }

    if( redrawScreenFunction != NULL )
    {
        (*redrawScreenFunction)(env) ;
    }
}
 ...
 *******************************************/
globle void EditorFunctionDefinition(void *env)
{
    ix_fn_define(env, "editor", 'v', PTR_FN EditCommand, "EditCommand", "*1k");
}
